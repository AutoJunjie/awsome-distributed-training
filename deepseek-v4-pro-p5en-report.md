...
### 6.1 最优配置（§10 Probe B = cuda-graph + EAGLE balanced，推荐用于生产）

```bash
# 镜像: lmsysorg/sglang:deepseek-v4-hopper
# 入口: sglang serve（必须；旧 python3 -m sglang.launch_server 在此镜像上 decode 塌成 BOS）
...
| 关键参数 | 来源/原因 |
|---|---|
| `SGLANG_ENABLE_SPEC_V2=1` env | cookbook DeepSeek-V4.mdx:146 "MTP currently requires SGLANG_ENABLE_SPEC_V2=1"；T5b 没加全崩 |
| `--cuda-graph-max-bs 8` | 显式小 bs 让 cg capture 通过（default=256 崩于 ipc_meta）；c=1 latency 4.73s → 1.00s |
| `--disable-custom-all-reduce` | `custom_all_reduce.cuh:350` 在 H200+TP=8 graph capture 时 invalid argument；关掉走 NCCL AR（NVLink，0 perf loss） |
| `--speculative-algo EAGLE --num-steps 1 --draft-tokens 2 --topk 1` | cookbook balanced recipe；accept_rate=96.6%；num_steps>1 目前 DSv4 radix backend 不支持 |
| `--moe-runner-backend marlin` | H200 FP4 必需（Marlin W4A16 kernel） |
| `--tool-call-parser deepseekv4 --reasoning-parser deepseek-v4` | DS-V4 chat template 必需；缺失则 decode 全 BOS |

### 6.2 基线配置（§4/§5 对应的 T4 初始部署，无 cuda-graph 无 EAGLE）
